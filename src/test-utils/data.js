...
export default function dataGenerator(config = {}) {
  const {
    primaryKeys = [
...
*/
  const uniqKeys = reduce(
    keyStore,
    (uniqueCombinations, primaryKeyOptions) =>
      flatMap(primaryKeyOptions, keyOption =>
        map(uniqueCombinations, intermediateCombo =>
          assign({}, keyOption, intermediateCombo)))
  );
...
*/
  ...
  for (let j = 0; j < length; j += 1) {
    ...
    for (let i = 0; i < uniqKeys.length; i += 1) {
      const rowObj = {};
      for (let k = 0; k < valueKeys.length; k += 1) {
...
*/
  const rows = flatMap(
    valueData,
    listOfValueKeys => map(listOfValueKeys, (valueKey, i) => assign({}, valueKey, uniqKeys[i]))
  );
